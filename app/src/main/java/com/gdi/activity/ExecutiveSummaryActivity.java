...
import com.gdi.R;
import com.gdi.adapter.ExecutiveSummaryAdapter;
...
import com.gdi.model.executivesummary.ExecutiveSummaryInfo;
import com.gdi.model.executivesummary.ExecutiveSummaryRootObject;
...
public class ExecutiveSummaryActivity extends BaseActivity implements DownloadPdfTask.PDFDownloadFinishedListner, DownloadExcelTask.DownloadExcelFinishedListner {
    ...
    private ExecutiveSummaryAdapter executiveSummaryAdapter;
    private ExecutiveSummaryInfo executiveSummaryInfo;
    private ArrayList<com.gdi.model.executivesummary.LocationsInfo> locationInfoArrayList;
    private static int REQUEST_FOR_WRITE_PDF = 1;
    private static int REQUEST_FOR_WRITE_EXCEL = 100;
    Context context;
    private static final String TAG = ExecutiveSummaryActivity.class.getSimpleName();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_executive_summary);
        context = this;
        ButterKnife.bind(ExecutiveSummaryActivity.this);
        initView();
    }
    ...
    private void initView() {
        ...
        list1 = (RecyclerView)findViewById(R.id.executive_summary_recycler);
        filterList();
    ...
    public void executiveSummaryList(){
        ...
        Response.Listener<String> stringListener = new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                ...
                try {
                    JSONObject object = new JSONObject(response);
                    if (!object.getBoolean(ApiResponseKeys.RES_KEY_ERROR)){
                        ExecutiveSummaryRootObject executiveSummaryRootObject = new GsonBuilder()
                                .create().fromJson(object.toString(),
                                        ExecutiveSummaryRootObject.class);
                        if (executiveSummaryRootObject.getData() != null &&
                                executiveSummaryRootObject.getData().toString().length() > 0){
                            executiveSummaryInfo = executiveSummaryRootObject.getData();
                            setExecutiveList();
                        }
        ...
        String auditUrl = ApiEndPoints.EXECUTIVESUMMARY + "?"
                + "brand_id=" + brandId + "&"
        ...
        ExecutiveSummaryRequest auditRequest = new
                ExecutiveSummaryRequest(AppPrefs.getAccessToken(context), auditUrl,
                stringListener, errorListener);
        VolleyNetworkRequest.getInstance(context).addToRequestQueue(auditRequest);
    }

    private void setExecutiveList(){
        locationInfoArrayList = new ArrayList<>();
        locationInfoArrayList.clear();
        locationInfoArrayList.addAll(executiveSummaryInfo.getLocations());
        AppLogger.e(TAG,"List Size: " +locationInfoArrayList.size());
        executiveSummaryAdapter = new ExecutiveSummaryAdapter(context, locationInfoArrayList);
    ...
    public void downloadPdf(final String url) {
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            ActivityCompat.requestPermissions(ExecutiveSummaryActivity.this,
                    new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_FOR_WRITE_PDF);

        } else {

            DownloadPdfTask downloadTask = new DownloadPdfTask(context, url, ExecutiveSummaryActivity.this);
        }
    }
    ...
    public void downloadExcel(final String url) {
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            ...
            DownloadExcelTask downloadTask = new DownloadExcelTask(context, url, ExecutiveSummaryActivity.this);
        }
    }
