...
public class ChangePasswordScreen extends BaseActivity {
    ...
    private void changePassword() {
        ...
        ChangePasswordRequest forgetPasswordRequest = new ChangePasswordRequest(username, otp.getText().toString(), newPassword.getText().toString(), stringListener, errorListener);
    ...
    private boolean validateInputs() {
        ...
        if (otp.getText().toString().length() <= 0) {
            validate = false;
            otp.setError(getString(R.string.enter_email));
        } else {
            otp.setError(null);
        }
